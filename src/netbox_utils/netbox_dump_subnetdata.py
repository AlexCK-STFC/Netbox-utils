...
class NetboxDumpSubnetdata(SCDNetbox):
    ...
    def _get_subnet_fields(self):
        ...
        tenants = [
            t.strip() for t in self.config["dump_subnetdata"]["tenants"].split(",")
        ]
        for prefix in self.netbox.ipam.prefixes.filter(
            tenant=tenants, family=4, children=0, vrf_id=None
        ):
            ...
            if (
                "gateway_ip" in prefix.custom_fields
                and prefix.custom_fields["gateway_ip"]
            ):
                gateway_ip = prefix.custom_fields["gateway_ip"]["address"].split(
                    "/", 2
                )[0]
    ...
    def write_subnetdata_txt(self, directory):
        ...
        for fields in subnet_fields:
            if "UDF" in fields:
                fields["UDF"] = ";".join(
                    [k + "=" + v for k, v in fields["UDF"].items()]
                )
        ...
        with open(
            os.path.join(directory, "subnetdata.txt"), "w", encoding="utf-8"
        ) as dumpfile:
    ...
    def write_subnetdata_json(self, directory):
        ...
        subnet_fields = self._get_subnet_fields()
        with open(
            os.path.join(directory, "subnetdata.json"), "w", encoding="utf-8"
        ) as dumpfile:

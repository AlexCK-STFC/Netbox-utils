...
class Netbox2Aquilon(SCDNetbox):
        ...
        if virtual_disks:
            ...
            for disk in virtual_disks:
                cmd = [
                    "add_disk",
                    "--machine",
                    f"{virtual_machine.aq_machine_name}",
                    "--disk",
                    f"{disk.name}",
                    "--controller",
                    "sata",
    ...
    def _netbox_copy_interfaces(self, device):
        ...
        for interface in interfaces:
            ...
            if (
                hasattr(interface, "mgmt_only")
                and interface.mgmt_only
                and not is_boot_interface
            ):
    ...
    def _netbox_get_personality(self, device, archetype, personality=None):
        if not personality:
            ...
            logging.debug(
                "Personality not specified, generating one from role and tenant"
            )
            if device.tenant:
        ...
        if self._call_aq(cmd_show_personality) != 0:
            logging.warning(
                'Personality "%s" not found, falling back to "inventory"', personality
            )
            personality = "inventory"
    ...
    def netbox_copy(self, opts):
        ...
        if not personality:
            logging.error(
                'Unable to determine personality of device "%s"', type(device)
            )
            sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        "--archetype",
        "-a",
        default=netbox2aquilon.config["aquilon"]["archetype"],
        help=(
    ...
    parser.add_argument(
        "--osname",
        default=netbox2aquilon.config["aquilon"]["osname"],
        help=(
    ...
    parser.add_argument(
        "--osversion",
        default=netbox2aquilon.config["aquilon"]["osversion"],
        help=(

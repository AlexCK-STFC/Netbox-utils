...
def test_get_device_by_hostname(mocker):
    ...
    scd_netbox.netbox.ipam.ip_addresses.filter = mocker.MagicMock(
        return_value=[deepcopy(fake_address_physical)]
    )
    scd_netbox.netbox.dcim.devices.get = mocker.MagicMock(
        return_value=deepcopy(FAKE.DEVICE_PHYSICAL)
    )
    ...
    scd_netbox.netbox.ipam.ip_addresses.filter.assert_called_with(
        dns_name="foo.example.org", family=4
    )
    scd_netbox.netbox.dcim.devices.get.assert_called_with(277636)

    # Test a virtual interface
    scd_netbox.netbox.ipam.ip_addresses.filter = mocker.MagicMock(
        return_value=[deepcopy(fake_address_virtual)]
    )
    scd_netbox.netbox.virtualization.virtual_machines.get = mocker.MagicMock(
        return_value=deepcopy(FAKE.DEVICE_VIRTUAL)
    )
    assert FAKE.DEVICE_VIRTUAL == scd_netbox.get_device_by_hostname("bar.example.org")
    scd_netbox.netbox.ipam.ip_addresses.filter.assert_called_with(
        dns_name="bar.example.org", family=4
    )
    ...
    scd_netbox.netbox.ipam.ip_addresses = SimpleNamespace()
    scd_netbox.netbox.ipam.ip_addresses.filter = mocker.MagicMock(
        return_value=[fake_address_garbage]
    )
    mocked_error = mocker.patch.object(logging, "error")
    with pytest.raises(SystemExit):
...
def test_get_rack_from_device(mocker):
    ...
    scd_netbox.netbox.dcim.racks.get = mocker.MagicMock(
        return_value=deepcopy(fake_rack)
    )
...
def test_get_interfaces_from_device(mocker):
    ...
    test_obj.netbox.dcim.interfaces.filter = mocker.MagicMock(
        return_value=deepcopy(FAKE.INTERFACES_PHYSICAL)
    )
    mocked_warning = mocker.patch.object(logging, "warning")
    assert FAKE.INTERFACES_PHYSICAL[:-1] == test_obj.get_interfaces_from_device(
        deepcopy(FAKE.DEVICE_PHYSICAL)
    )
    mocked_warning.assert_called()

    # Test virtual interfaces
    test_obj.netbox.virtualization.interfaces = SimpleNamespace()
    test_obj.netbox.virtualization.interfaces.filter = mocker.MagicMock(
        return_value=deepcopy(FAKE.INTERFACES_VIRTUAL)
    )
    assert FAKE.INTERFACES_VIRTUAL == test_obj.get_interfaces_from_device(
        deepcopy(FAKE.DEVICE_VIRTUAL)
    )
...
def test_get_addresses_from_interface(mocker):
    ...
    assert FAKE.ADDRESSES_IPV4 == scd_netbox.get_addresses_from_interface(
        FAKE.INTERFACES_PHYSICAL[0]
    )
    ...
    assert (
        len(
            scd_netbox.get_addresses_from_interface(
                SimpleNamespace(count_ipaddresses=42)
            )
        )
        == 0
    )
    ...
    scd_netbox.netbox.ipam.ip_addresses = SimpleNamespace()
    scd_netbox.netbox.ipam.ip_addresses.filter = mocker.MagicMock(
        return_value=deepcopy(FAKE.ADDRESSES_IPV6)
    )
    mocked_warning = mocker.patch.object(logging, "warning")
    assert (
        len(
            scd_netbox.get_addresses_from_interface(
                SimpleNamespace(count_ipaddresses=2112)
            )
        )
        == 0
    )

...
def test__netbox_copy_interfaces(mocker, mock_netbox2aquilon, make_nb_obj):
    ...
    add_bmc0 = add_interface_base_cmd + [
        "--interface",
        "bmc0",
        "--mac",
        "A1:B2:C3:D4:E5:3F",
    ...
    add_eth0 = add_interface_base_cmd + [
        "--interface",
        "eth0",
        "--mac",
        "A1:B2:C3:D4:E5:DA",
    ]
    add_eth1 = add_interface_base_cmd + [
        "--interface",
        "eth1",
        "--mac",
        "A1:B2:C3:D4:E5:DB",
    ...
    assert (
        add_interface_base_cmd + ["--interface", "bond0", "--iftype", "bonding"] in cmds
    )
    assert (
        add_interface_base_cmd + ["--interface", "bond1", "--iftype", "bonding"] in cmds
    )
    assert (
        add_interface_base_cmd + ["--interface", "eth0", "--mac", "A1:B2:C3:69:2A:A1"]
        in cmds
    )
    assert (
        add_interface_base_cmd + ["--interface", "eth1", "--mac", "A1:B2:C3:69:2A:A2"]
        in cmds
    )
    assert (
        add_interface_base_cmd + ["--interface", "eth2", "--mac", "D4:E5:3F:52:37:8D"]
        in cmds
    )
    assert (
        add_interface_base_cmd + ["--interface", "eth3", "--mac", "D4:E5:3F:52:37:8E"]
        in cmds
    )

    assert update_interface_base_cmd + ["--interface", "bond0", "--boot"] in cmds
    assert (
        update_interface_base_cmd + ["--interface", "eth0", "--master", "bond1"] in cmds
    )
    assert (
        update_interface_base_cmd + ["--interface", "eth1", "--master", "bond1"] in cmds
    )
    assert (
        update_interface_base_cmd + ["--interface", "eth2", "--master", "bond0"] in cmds
    )
    assert (
        update_interface_base_cmd + ["--interface", "eth3", "--master", "bond0"] in cmds
    )
